...
// ========================================================================== //
APInt IntraProceduralRA::getMin()
{
	return Min;
}

APInt IntraProceduralRA::getMax()
{
	return Max;
}

Range IntraProceduralRA::getRange(const Value *v){
...
Cousot IntraProceduralRA::run(Function &F) {
//	if(CG) delete CG;
	CG = new Cousot();
...
IntraProceduralRA::~IntraProceduralRA(){
}

// ========================================================================== //
// InterProceduralRangeAnalysis
// ========================================================================== //
APInt InterProceduralRA::getMin()
{
	return Min;
}

APInt InterProceduralRA::getMax()
{
	return Max;
}

Range InterProceduralRA::getRange(const Value *v){
	return CG->getRange(v);
}

unsigned InterProceduralRA::getMaxBitWidth(Module &M) {
	unsigned max = 0;
...
void InterProceduralRA::MatchParametersAndReturnValues(Function &F,
		ConstraintGraph &G) {
...
InterProceduralRA::~InterProceduralRA(){
}
...
void ConstraintGraph::buildValueBranchMap(const BranchInst *br) {
	...
	if (ConstantInt * CI = dyn_cast<ConstantInt>(ici->getOperand(1))) {
		...
		ConstantRange tmpT = ConstantRange::makeSatisfyingICmpRegion(pred, CR);
